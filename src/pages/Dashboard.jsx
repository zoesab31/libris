        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-12 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
            <div className="max-w-5xl mx-auto">
              ...
              {/* Last Friend Finished Book */}
              {friendsFinishedBooks.length > 0 && (() => {
                const lastFriendBook = friendsFinishedBooks[0];
                const friendBook = allBooks.find(b => b.id === lastFriendBook.book_id);
                const friend = myFriends.find(f => f.friend_email === lastFriendBook.created_by);
                
                if (!friendBook || !friend) return null;
                
                return (
                  <Card className="shadow-2xl border-0 overflow-hidden hover-lift card-glow mb-6" style={{ backgroundColor: 'white' }}>
                    <div className="h-2" style={{ background: 'linear-gradient(90deg, var(--lavender), var(--soft-pink))' }} />
                    <CardHeader className="pb-3">
                      <CardTitle className="flex items-center gap-2" style={{ color: 'var(--dark-text)' }}>
                        <Users className="w-5 h-5" />
                        Dernier livre terminé par {friend.friend_name?.split(' ')[0] || 'une amie'}
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                      <div className="flex gap-4">
                        <div className="w-24 h-36 rounded-xl overflow-hidden shadow-lg flex-shrink-0 animate-pulse-slow"
                             style={{ backgroundColor: 'var(--beige)' }}>
                          {friendBook.cover_url ? (
                            <img src={friendBook.cover_url} alt={friendBook.title} 
                                 className="w-full h-full object-cover" />
                          ) : (
                            <div className="w-full h-full flex items-center justify-center">
                              <BookOpen className="w-8 h-8" style={{ color: 'var(--warm-pink)' }} />
                        ...
                        <div className="flex-1">
                          <span className="text-xs px-3 py-1 rounded-full font-bold mb-3 inline-block"
                            ...
                            <div className="flex items-center gap-1 mb-2">
                              {Array.from({ length: 5 }).map((_, i) => (
                                <Star
                                  ...
                                  className="w-4 h-4"
                                  style={{
                                    fill: i < lastFriendBook.rating ? 'var(--gold)' : 'none',
                                    stroke: 'var(--gold)',
                                  }}
                                />
                              ))}
                          ...
                          {lastFriendBook.end_date && (
                            <p className="text-xs" style={{ color: 'var(--warm-brown)' }}>
                              Terminé le {format(new Date(lastFriendBook.end_date), 'dd MMMM yyyy', { locale: fr })}
                            </p>
                          )}
                        </div>
                      </div>
